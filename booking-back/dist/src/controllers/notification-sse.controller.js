...
class NotificationSSEController {
    ...
    async getConnectedUsers(req, res) {
        try {
            const connectedUsers = notification_sse_service_js_1.default.getConnectedUsers();
            const count = notification_sse_service_js_1.default.getConnectedUserCount();
            res.json({
                connectedUsers,
                count,
            });
    ...
    async broadcastToUser(req, res) {
        try {
            ...
            const sent = notification_sse_service_js_1.default.broadcastToUser(userId, notification);
            res.json({
    ...
    async broadcastToUsers(req, res) {
        try {
            ...
            const results = notification_sse_service_js_1.default.broadcastToUsers(userIds, notification);
            res.json({
    ...
    async isUserConnected(req, res) {
        try {
            ...
            const connected = notification_sse_service_js_1.default.isUserConnected(userIdString);
            res.json({
                userId,
                connected,
            });
