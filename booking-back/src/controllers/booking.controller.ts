 ...
 */
export async function sendBookingConfirmationByPlan(
  businessId: string,
      ...
      try {
        if (alreadyVerified) {
          await sendBookingConfirmationByPlan(
            ...
            needsEmailVerification ? verificationToken! : undefined
          )
        }
      } catch (notifyError) {
        console.error('[v0] Failed to send booking confirmation:', notifyError)
  ...
  */
  ...
  async getBusinessBookings(req: Request, res: Response): Promise<void> {
    try {
      ...
      const validStatuses: BookingStatus[] = ['PENDING',"UNVERIFIED",'COMPLETED', 'CONFIRMED', 'CANCELLED']
